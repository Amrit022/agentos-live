...
//|                         Forex Master Pro - Auto Trading EA v3.0  |
//|                                    Built for OctaFX MT5          |
//+------------------------------------------------------------------+
//
//  VERSION 3.0 (ULTIMATE) UPGRADES:
//    1. Multi-Timeframe Filter (H4 + Daily trend confirmation)
//    2. Break-Even Stop (risk-free trades after 1x ATR profit)
//    3. Daily Drawdown Protection (stops trading after 3% daily loss) - [REMOVED]
...
//    8. Virtual Account Circuit Breaker (stops all trading if float loss > 5%)
//
//  HOW TO INSTALL:
...
//
//+------------------------------------------------------------------+
#property copyright "Forex Master Pro EA v3.2"
#property version   "3.2"
#property description "Ultimate confluence-based forex auto-trader with 7 pro upgrades"
...
input int     MinSignalScore     = 2;        // Minimum confluence score (1-5)
...
input int     SignalCooldownBars = 5;        // Reduced from 10 to 5 bars for quicker subsequent setups
...
// ULTIMATE UPGRADE 4: Virtual Account Circuit Breaker
...
{
   Print("===========================================");
   Print("  FOREX MASTER PRO EA v3.2 (ULTIMATE) - INITIALIZED");
   Print("  7 PRO UPGRADES ACTIVE (Drawdown Removed)");
...
void OnDeinit(const int reason)
{
   ...
   Comment("");
   Print("Forex Master Pro EA v2.0 - Removed");
}

//+------------------------------------------------------------------+
//| ULTIMATE UPGRADE 4: Account Circuit Breaker                       |
//+------------------------------------------------------------------+
void CheckCircuitBreaker()
{
   if(!UseCircuitBreaker || circuitBreakerTripped) return;
   ...
   if(floatLossPct >= MaxFloatingLossPct)
   {
      ...
      Print("EMERGENCY CIRCUIT BREAKER TRIGGERED! Floating Loss: -", DoubleToString(floatLossPct, 2), "% (Limit: ", MaxFloatingLossPct, "%). Closing all positions.");
      Alert("ForexPro EA: Emergency Circuit Breaker triggered! Closing all trades.");
...
void OnTick()
{
   ...
   int requiredScore = isGold ? 4 : MinSignalScore;
   ...
   if(buySignal && openTrades < MaxOpenTrades)
   {
      ...
      if(trade.Buy(lotSize, _Symbol, ask, 0, 0, comment))
      {
         ...
         Print("BUY opened | Score: ", buyScore, "/5 | Lot: ", lotSize,
               " | SL: ", sl, " | TP1: ", tp1Price, " | TP2: ", tp,
               " | ADX: ", DoubleToString(adx[0], 1),
               " | MTF: ", (mtfBullish ? "ALIGNED" : "N/A"));
         Alert("ForexPro EA: BUY ", _Symbol, " @ ", ask);
      }
      else
   ...
   if(sellSignal && openTrades < MaxOpenTrades)
   {
      ...
      if(trade.Sell(lotSize, _Symbol, bid, 0, 0, comment))
      {
         ...
         originalLotSize = lotSize;
         partialTPTaken = false;
         Print("SELL opened | Score: ", sellScore, "/5 | Lot: ", lotSize,
               " | SL: ", sl, " | TP1: ", tp1Price, " | TP2: ", tp,
               " | ADX: ", DoubleToString(adx[0], 1),
               " | MTF: ", (mtfBearish ? "ALIGNED" : "N/A"));
         Alert("ForexPro EA: SELL ", _Symbol, " @ ", bid);
      }
      else
...
void UpdateDashboard()
{
   ...
   // Build dashboard
   string d = "";
   d += "=== FOREX MASTER PRO v3.2 (ULTIMATE) ===\n";
   if(isGold)
   {
   ...
   d += "Circuit B.: " + (!UseCircuitBreaker ? "Disabled" : (circuitBreakerTripped ? "TRIPPED!" : "Active")) + "\n";
